    ...
    def forward(self, x, mean=None, std=None):
        ...
        for model in self.models:
            if isinstance(model, ResNetModel):
                ...
                # Use all inputs for CNN
                out = model(x, mean, std)
            else:
                # For other models, use only the image input
                out = model(x)
            outputs.append(out)
...
def main():
    ...
    # Load trained models
    cnn_model = load_trained_model(CNN, "saved_model/CNN_2024_09_14-16_07_45.pt", device)
    resnet_model = load_trained_model(ResNetModel, "saved_model/ResNetWithExtras_2024_09_13-19_49_49.pt", device)
    densenet_model = load_trained_model(DenseNetModel, "saved_model/DenseNetModel_2024_09_14-18_42_26.pt", device)
    inception_model = load_trained_model(InceptionV3Model, "saved_model/InceptionV3Model_2024_09_15-12_43_19.pt", device)
    ...
    # Define criterion (assuming CrossEntropyLoss, adjust if needed)
    criterion = nn.CrossEntropyLoss()

    # Make predictions using the ensemble model and the common predict function
    print("Testing the ensemble model...")
    test_loss, test_accuracy, predictions, ground_truths = predict(ensemble_model, test_loader, criterion, device, eval=True)

    print(f"Ensemble Model - Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}")

    # Generate and save the classification report
    
    class_to_idx = train_loader.dataset.subset.dataset.class_to_idx
    save_classification_report(ground_truths, predictions, class_to_idx, filename='ensemble_classification_report.txt')
    
    # Plot confusion matrix
    plot_confusion_matrix(ground_truths, predictions, class_to_idx, filename='ensemble_confusion_matrix.png')
