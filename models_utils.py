...
def split_dataset_func(dataset, val_split, test_split):
    # Group images by patient and class
...
def get_data_loaders(hparams):
    ...
    # Set test_split to 0.2 (20% of the dataset)
    test_split = 0.25
    val_split = hparams.val_split * (1 - test_split)  # Adjust validation split accordingly
...
def validate(model, val_loader, criterion, device):
    ...
    val_loss /= val_length
    val_accuracy = 100. * val_correct / val_length
    f1 = f1_score(targets, preds, average='macro')
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs, early_stopping=None):
    train_losses, train_accuracies, train_f1_scores = [], [], []
    val_losses, val_accuracies, val_f1_scores = [], [], []

    for epoch in range(num_epochs):
        # Training
        train_loss, train_accuracy, train_f1 = train(model, train_loader, criterion, optimizer, device, epoch, num_epochs)
